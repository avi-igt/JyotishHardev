...
const HomePage: NextPage = () => {
  ...
  const handleEmail = () => {
    ...
    const luckyLine = lucky
      ? `Lucky Numbers: ${lucky.numbers.join(', ')} | Lucky Colour: ${lucky.colour} | Lucky Day: ${lucky.day}`
      : '';
    const subject = encodeURIComponent(`Kundli for ${result.name}`);
    const body = encodeURIComponent(
      `Vedic Birth Chart — ${result.name}\n` +
      `${'─'.repeat(40)}\n` +
      `Lagna (Ascendant): ${result.lagna}\n` +
      `Rashi (Moon Sign): ${result.moon_sign}\n` +
      `Nakshatra: ${result.nakshatra}\n` +
      `Current Dasha: ${result.current_dasha}\n` +
      (luckyLine ? `${luckyLine}\n` : '') +
            ...
            <div className="results">
              <div className="results-header">
                <div className="results-actions">
                  ...
                  <button className="share-btn" onClick={handleEmail}>
                    Email ↗
                  </button>
                </div>
